		...
		<div class="fl-container">
			<div class="fl-title-page" >
				<span style="font-size:20px">				
					...
					Input Tingkat Kesehatan Perusahaan 
				</span>
			</div>
			
		<form id="form_tkp" action="/txtkp/tkp_save" method="POST">	
			<div class="col-md-12 fl-table">
				...
				<div class="uk-overflow-auto">
					...
					<thead>
						...
						<input type="hidden" name="tahun[]" id="tahun[]" value="{{ $years }}">
						<tbody>
							<tr>
								...
								<td><input type="hidden" name="hasilratio[]" id="hasilratio[]" value="{{ $hasil[$i] }}">{{ $hasil[$i] }}</td>
								<td>
									@if($data->TIPE_TKP != 'Operasional')
										<input type="hidden" name="pencapaian[]" id="pencapaian[]" value="{{ $pencapaian[$i] }}"><span style="color:white">{{ $pencapaian[$i] }}</span>
									@else
										<input type="hidden" name="pencapaian[]" id="pencapaian[]" value="{{ $pencapaian[$i] }}"><span style="color:black">{{ $pencapaian[$i] }}</span>
									@endif
								</td>
					...
					@endforeach
						...
						<tbody>
							<tr>
								...
								<td>
									<button class="uk-button uk-button-danger fl-button" type="button" onclick="location.href = '{{ url('txtkp')}}'">Cancel</button>
								</td>
							</tr>
						</tbody>
					</table>

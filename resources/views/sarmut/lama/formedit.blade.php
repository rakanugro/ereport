...
<body>
	...
	<div class="fl-main-container">
		...
		<div class="fl-container">
			...
			<div class="fl-table">
				<form id="form_mst_sarmut" action="/sarmut/master_sarmut_edit" method="POST">
				{{ csrf_field() }}
					...
					<div>
                        ...
                        <select id="indicator_list" class="form-control select2-list" name="indicator_list[]">
                            ...
                            @foreach($indicator_list as $indicator)
                                <option value="{{ $indicator->INDICATOR_ID }}" {{ $selectedindicator == $indicator->INDICATOR_ID ? 'selected="selected"' : '' }}>{{ $indicator->SUB_DIVISION_CODE }} | {{ $indicator->INDICATOR_NAME }}</option>
                            @endforeach
                        </select>
					<div>
					<div>

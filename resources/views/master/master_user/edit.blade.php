...
<body>
	...
	<div class="fl-main-container">
		...
		<div class="fl-container">
			...
			<div class="fl-table">
				<form id="form_blade" action="/master/master_user_edit_save" method="POST">
					{{ csrf_field() }}
					<div class="form-group m-form__group row">
						...
						<div class="col-lg-4">
							...
							<input type="text"  class="form-control m-input" name="nipp" id="nipp" value="{{ $nipp }}" onchange="check_username()" required>
					...
					<div class="form-group m-form__group row">
						<div class="col-lg-4">
							...
							<input type="text"  class="form-control m-input" name="nama" id="nama" value="{{ $nama }}" required>
						</div>
					</div>
					<div class="form-group m-form__group row">
						<div class="col-lg-4">
							...
							<input type="email" class="form-control m-input" name="email" id="email" value="{{ $email }}" required>
						</div>
					</div>
					<div class="form-group m-form__group row">
						<div class="col-lg-4">
							...
							<select class="form-control select2-list" name="access"   id="access" required>
								...
								<option value="DVP SUB DIVISI" {{ $access == "DVP SUB DIVISI" ? 'selected' : '' }}>DVP/DGM Sub Divisi</option>
								<option value="VP SUB DIVISI" {{ $access == "VP SUB DIVISI" ? 'selected' : '' }}>VP/GM Sub Divisi</option>
								...
								<option value="REPORT_ONLY" {{ $access == "REPORT_ONLY" ? 'selected' : '' }}>Untuk Report Saja</option>
							</select>
						</div>
					</div>
					<div class="form-group m-form__group row">
						<div class="col-lg-4">
							...
							<select class="form-control select2-list" id="organize_struct_list" name="organize_struct_list" required>
								...
								<option value="{{ $org->ORGANIZATION_STRUCTURE_ID }}" selected>{{ $org->BRANCH_OFFICE_NAME }} | {{ $org->SUB_DIVISION_NAME }}</option>
								@else
					...
					<div>
						<button class="uk-button uk-button-primary fl-button" onclick="save_edit_user(this.form.id);return false;">
	...
	<script>
		...
		function save_edit_user(formid)
	    {   
			submit_form(formid);
		}
